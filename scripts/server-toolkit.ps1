...
$StartButton.Add_Click({
    ...
    if ($hostKeyChanged) {
        ...
        try {
            ...
            $AppendLog.Invoke("Old PuTTY host key removed. Accepting new key...`r`n")
        } catch {
            $AppendLog.Invoke("[ERROR] Unable to remove old PuTTY host key: $_`r`n")
        }

        # Also clean matching entries from OpenSSH known_hosts (ssh.exe)
        try {
            $knownHostsPath = Join-Path (Join-Path $env:USERPROFILE ".ssh") "known_hosts"
            if (Test-Path $knownHostsPath) {
                $allLines    = Get-Content -Path $knownHostsPath
                ...
                # Examples we want to remove:
                #   65.108.87.84 ssh-ed25519 AAAA...
                #   [65.108.87.84]:22 ssh-ed25519 AAAA...
                $pattern1 = "^$escapedHost\s"
                $pattern2 = "^\[$escapedHost\]:$escapedPort\s"
